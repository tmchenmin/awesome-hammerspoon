    ...
    if safari_running then
        ...
        if stat then
            chooser_data = hs.fnutils.imap(data, function(item)
                return {text=item[1], subText=item[2], image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/safari.png")), outputType="safari", url=item[2]}
            end)
        end
    end
...
function browserSource()
    local browsersource_overview = {text="Type t ⇥ to search safari/chrome Tabs.", image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/tabs.png")), sourceKeyword="t"}
    ...
    function browserFunc()
        ...
        search_chooser:choices(chooser_data)
        browserTabsRequest()
        local source_desc = {text="Browser Tabs Search", subText="Search and select one item to open in corresponding browser.", image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."./resources/tabs.png"))}
        table.insert(chooser_data, 1, source_desc)
        search_chooser:choices(chooser_data)
        search_chooser:queryChangedCallback()
...
function youdaoInstantTrans(querystr)
    ...
    if string.len(querystr) > 0 then
                    ...
                    if decoded_data.errorCode == 0 then
                        ...
                        if #dictpool > 0 then
                            chooser_data = hs.fnutils.imap(dictpool, function(item)
                                return {text=item, image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/youdao.png")), outputType="clipboard", clipText=item}
                            end)
                            search_chooser:choices(chooser_data)
        ...
        chooser_data = {}
        local source_desc = {text="Youdao Dictionary", subText="Type something to get it translated …", image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/youdao.png"))}
        table.insert(chooser_data, 1, source_desc)
        search_chooser:choices(chooser_data)
    end
end
...
function youdaoSource()
    local youdaosource_overview = {text="Type y ⇥ to use Yaodao dictionary.", image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/youdao.png")), sourceKeyword="y"}
...
function appKillSource()
    ...
    local appkillsource_overview = {text="Type k ⇥ to Kill running process.", image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/taskkill.png")), sourceKeyword="k"}
    ...
    function appkillFunc()
        ...
        appsInfoRequest()
        -- More tips
        local source_desc = {text="Kill Processes", subText="Search and select some items to get them killed.", image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/taskkill.png"))}
        table.insert(chooser_data, 1, source_desc)
...
function thesaurusRequest(querystr)
    ...
    if string.len(querystr) > 0 then
                ...
                if pcall(function() hs.json.decode(data) end) then
                    ...
                    if #decoded_data > 0 then
                        chooser_data = hs.fnutils.imap(decoded_data, function(item)
                            return {text = item.word, image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/thesaurus.png")), outputType="keystrokes", typingText=item.word}
                        end)
                        search_chooser:choices(chooser_data)
                        search_chooser:refreshChoicesCallback()
                    end
        ...
        chooser_data = {}
        local source_desc = {text="Datamuse Thesaurus", subText="Type something to get more words like it …", image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/thesaurus.png"))}
        table.insert(chooser_data, 1, source_desc)
        search_chooser:choices(chooser_data)
    end
end
...
function thesaurusSource()
  local thesaurus_overview = {text="Type s ⇥ to request English Thesaurus.", image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/thesaurus.png")), sourceKeyword="s"}
...
function MenuitemsSource()
    local menuitems_overview = {text="Type m ⇥ to search Menuitems.", image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/menus.png")), sourceKeyword="m"}
    table.insert(chooserSourceOverview,menuitems_overview)
    function menuitemsFunc()
        MenuitemsRequest()
        local source_desc = {text="Menuitems Search", subText="Search and select some menuitem to get it clicked.", image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/menus.png"))}
        table.insert(chooser_data, 1, source_desc)
        search_chooser:choices(chooser_data)
        search_chooser:queryChangedCallback()
        search_chooser:searchSubText(true)
            ...
            if pcall(function() hs.json.decode(data) end) then
                ...
                if #decoded_data > 0 then
                    chooser_data = hs.fnutils.imap(decoded_data, function(item)
                        ...
                        return {text=item.title, subText=final_content, image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/v2ex.png")), outputType="browser", url=item.url}
                    end)
                    local source_desc = {text="v2ex Posts", subText="Select some item to get it opened in default browser …", image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/v2ex.png"))}
                    table.insert(chooser_data, 1, source_desc)
                    search_chooser:choices(chooser_data)
                    search_chooser:refreshChoicesCallback()
                end
...
function v2exSource()
    local v2ex_overview = {text="Type v ⇥ to fetch v2ex posts.", image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/v2ex.png")), sourceKeyword="v"}
...
function emojiRequest(querystr)
    ...
    if string.len(querystr) > 0 then
        ...
        chooser_data = {}
        local source_desc = {text="Relevant Emoji", subText="Type something to find relevant emoji from text …", image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/emoji.png"))}
        table.insert(chooser_data, 1, source_desc)
        search_chooser:choices(chooser_data)
    end
end
...
function emojiSource()
  local emoji_overview = {text="Type e ⇥ to find relevant Emoji.", image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/emoji.png")), sourceKeyword="e"}
...
function timeRequest()
    ...
    chooser_data = hs.fnutils.imap(hs_time_commands, function(item)
        ...
        return {text=exec_result, subText="date "..item, image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/time.png")), outputType="keystrokes", typingText=exec_result}
    end)
end
...
function timeDeltaRequest(querystr)
    if string.len(querystr) > 0 then
        ...
        if #valid_inputs > 0 then
            ...
            timeRequest()
            local source_desc = {text="Date Query", subText="Type +/-1d (or y, m, w, H, M, S) to query date forward or backward.", image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/time.png"))}
            table.insert(chooser_data, 1, source_desc)
            search_chooser:choices(chooser_data)
        end
    else
...
function timeSource()
    local time_overview = {text="Type d ⇥ to format/query Date.", image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/time.png")), sourceKeyword="d"}
...
function justNoteRequest()
    ...
    if #hs_justnote_history == 0 then
        chooser_data = {{text="Write something and press Enter.", subText="Your notes is automatically saved, selected item will be erased.", image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/justnote.png"))}}
    else
        chooser_data = hs.fnutils.imap(hs_justnote_history, function(item)
            return {uuid=item.uuid, text=item.content, subText=item.ctime, image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/justnote.png")), outputType="noteremove"}
        end)
    end
end
...
function justNoteSource()
    local justnote_overview = {text="Type n ⇥ to Note something.", image=hs.image.imageFromPath(hs.fs.pathToAbsolute(hs.configdir.."/resources/justnote.png")), sourceKeyword="n"}
